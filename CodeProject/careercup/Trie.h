...
private:
    Node* insert(Node* cur, string str, int l) {
        ...
        if (l == str.length()) { cur->end = true; return cur; }
    ...
    bool remove(Node*& cur, string str, int l) {
        ...
        if (l == str.length()) {
            cur->end = false;  return isLeaf(cur);
        }
    ...
    void prefix(Node* cur, string str, int l, vector<string>& result) {
        if (l == str.length()) { result.push_back(str); return; }
        if (cur == nullptr) {
            result.push_back(str.substr(0, l - 1)); return;
        }
        if (cur->end) result.push_back(str.substr(0, l));
