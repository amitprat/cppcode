...
public:
    static void test() {
        ...
        for (auto& test : tests) {
            ...
            cout << "MinLengthSubarray for sum:" << test.second << " :: (" << res.second.first << "," << res.second.second << ")=" << res.first << "[";
            for (int i = res.second.first; i <= res.second.second && res.second.second != INT_MAX; i++) cout << test.first[i] << " ";
            cout << "]" << endl;
        }
    }
    ...
    pair<int, pair<int, int>> getMinimumSizeSubArray(vector<int>& a, int s) {
        int l = 0, r = 0;
        pair<int, pair<int, int>> result = { 0,{0,INT_MAX} };
        int curSum = 0;
        while (l < a.size()) {
            if (curSum >= s && l < r) {
